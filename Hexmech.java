...
#define HEXNORTHEAST 5
     ...
     */
    //calculating middle location of circle for circles of influence
    public static Point hexToPixel(int x, int y) {
        int mx = (int) (x * (s + t) - (BORDERS)+3);
        int my = (int) (y * h + (x % 2) * r - BORDERS+3);
    ...
    public static Polygon hex (int x0, int y0) {
        ...
        int topY = y0 + BORDERS;
        int leftX = x0+BORDERS; // + (XYVertex ? t : 0); //Fix added for XYVertex = true.
...
//I think that this XYvertex stuff is taken care of in the int x line above. Why is it here twice?
        ...
        // else
        cx = new int[] {leftX + w / 2,leftX + w,leftX + w,leftX + w / 2,leftX,leftX};	//this is for the whole hexagon to be below and to the right of this point

        cy = new int[] {topY,topY + h / 4,topY + h / 4 * 3,topY + h,topY + h / 4 * 3,topY + h / 4};
